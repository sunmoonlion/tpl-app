#!/bin/bash
# celeryworker-tpl-admin-backend Celery Worker Secret YAML 生成脚本

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/generate-celeryworker-tpl-admin-backend-secret.conf"
...
main() {
    log_info "开始生成 celeryworker-tpl-admin-backend Celery Worker Secret YAML 文件..."
