#!/bin/bash
# nodebullworker-tpl-web-backend Node Bull Worker ConfigMap YAML 生成脚本

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
CONFIG_FILE="$SCRIPT_DIR/generate-nodebullworker-tpl-web-backend-config.conf"
...
main() {
    log_info "开始生成 nodebullworker-tpl-web-backend Node Bull Worker ConfigMap YAML 文件..."
