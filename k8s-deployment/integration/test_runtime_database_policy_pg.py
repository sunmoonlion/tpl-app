...
async def database():
    assert os.environ.get("RUNTIME_POLICY_TEST_CONFIRM") == "disposable-b7o-only"
    ...
    try:
        ...
        async with engines["migration"].begin() as connection:
            ...
            statements = policy.template_grants(
                schema="public", principals=names, columns=columns
            )
